...
$trans = invoke-expression -command "c:\apps\php\php.exe c:\pgo\scripts\pgo.php printnum"
foreach ( $app in $apps )  {
	$num = 0
	foreach ( $t in $trans )  {
		$t = $t.split(":")
			if ( $t[0] -eq $app )  {
				...
				iisreset /stop
				iisreset /start
				invoke-expression -command "c:\apps\php\php.exe c:\pgo\scripts\pgo.php vhost 80 $t[0]"
				start-sleep -s 10
		}
	}
}
